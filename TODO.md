...
### 54. DFS Engine — Layer 1 projection edge study — ✅ DONE — VERDICT: **STOP** (2026-06-24, branch `feat/dfs-projection-edge-study-verdict`)

**VERDICT (2026-06-24): STOP — well-powered.** TODO #40 blocker resolved (re-ingested raw pbp/ngs/snap_counts/depth_charts/schedules/draft_picks + rebuilt the full 2018–2024 feature cache, Vegas columns populated), Sleeper weekly 2020–2024 ingested, study run on 2021–2024 → `reports/dfs_projection_edge_2026-06-24.md`. On the disagreement head-to-head our model is closer to actual only **0.476** of the time (95% CI 0.456–0.495, clustered by player-season; Sleeper wins ~52%), robust to by-week (0.456–0.496) and bonuses (0.451–0.489); ranking skill also worse (Spearman −0.031, CI −0.038…−0.023). Well-powered: 1006 disagreement clusters (≥ N_MIN=100), CI half-width ≈0.02 (< 0.05) — STOP, not INCONCLUSIVE. Loss concentrated in QB (0.431)/RB (0.430); TE ~even (0.498); **WR a bright spot (0.535, we beat Sleeper)**; 50/50 ours+Sleeper blend 0.543 but INCONCLUSIVE (CI 0.485–0.600). **Gate implication: do NOT build DFS Layer 2 on this edge** — the model can't beat even Sleeper-alone (a softer proxy than the sharp DFS field). **Per-position significance (2026-06-25, `reports/dfs_per_position_significance_2026-06-25.md`):** the per-position fractions track modeling effort — **WR (`ensemble-decomposed`) CI 0.504–0.566 is a statistically REAL edge**; RB (`baseline`) 0.397–0.463 is a real deficit AND the biggest drag (largest subset); QB (`lightgbm-nb`) 0.375–0.493 a real deficit (modeled, still loses); TE (`baseline`) 0.438–0.550 not significant. So STOP is a verdict on a **half-built model** (2/4 positions are no-effort baseline), not a ceiling. If DFS is ever revisited, RB is the highest-leverage modeling target (un-modeled + biggest subset); QB is the cautionary case. Universe persisted to `data/dfs_universe_2021-2024.parquet`; `scripts/_dfs_per_position_analysis.py` regenerates the report (the ~75-min cost is the single-process walk-forward emit, not contention). **Open follow-up (non-gating):** the formal `calibrate --prior-season 2020` diagnostic is blocked — it needs features back to 2017 (EnsembleModel ≥3 training seasons), and rebuilding 2017 features trips a latent **schema-too-strict bug**: `opp_allowed_{qb,rb,wr,te}_fppg_l4` carry a `ge=0` check but an early-season trailing-4 average is legitimately negative (a QB can post net-negative fantasy pts). 2018–2024 never hit it; 2017 does. Relaxing the schema invariant needs a decision (CLAUDE.md schema discipline); non-gating since the study's realized power already confirms the constants' regime. Also fixed a shipped CLI bug: `calibrate` was missing `--features-root`. Original harness status below for history.

First DFS Engine slice: a retrospective kill-test — does our home-grown weekly model (alone or blended with Sleeper) beat Sleeper's own weekly projections under DraftKings base scoring, judged by a single **pre-registered** gate (disagreement head-to-head, player-season **clustered bootstrap**, anti-masking + by-week robustness + bonus sensitivity → ADOPT/STOP/INCONCLUSIVE)? Spec/plan `docs/superpowers/specs|plans/2026-06-23-dfs-projection-edge-study.*`. Built via the full `superpowers-go` pipeline (spec-review 4 iters → plan-review 2 iters → 12-task subagent-driven implementation, each TDD + dual review).

**Shipped + unit-tested:** `season_calendar` (TODO #41 era-week hoist), `normalize_join_id` lifted to `ingest/identity.py` (TODO #38 seam), `Ruleset.draftkings()` + `dk_actuals_bonus`, `ExternalProjectionWeeklySchema` + Sleeper **weekly** ingest, `dfs/` package (`actuals`, `projections` walk-forward emitter + a **non-vacuous** trajectory/vegas leakage guard, `blend`, `edge_study` metric harness, `run` orchestrator) + `scripts/dfs_edge_study.py` CLI.
...
### 55. DFS — model RB off `baseline` (the highest-leverage lead from the #54 STOP) — **NEXT**

**Why this is the lead.** The #54 per-position significance run (`reports/dfs_per_position_significance_2026-06-25.md`) showed the edge tracks modeling effort: WR (`ensemble-decomposed`) is a statistically real edge over Sleeper (0.535, CI 0.504–0.566), while **RB runs the no-effort `baseline` model and is the single biggest drag on the pooled STOP** — real deficit (0.430, CI 0.397–0.463) AND the largest disagreement subset (1009 cells / 356 clusters). RB dominates the pool by sheer volume, so lifting it is the most leverage we have to move the pooled verdict off STOP. (TE is also baseline but ~neutral/small; QB already has a real model and still loses — the cautionary case that says this is a *bet*, not a guaranteed flip.)

**The work.** Give RB the WR-style treatment — a decomposed/ensemble model instead of `baseline` — i.e. build/borrow an `ensemble-decomposed` (or `lightgbm-nb`) RB factory and flip `POSITION_DISPATCH[Position.RB].default_model_class` once it beats baseline on the existing model backtest. Then re-run the edge study and re-check both the pooled verdict and RB's per-position fraction. **Reuse, don't rebuild:** the WR ensemble (`models/ensemble.py`, the decomposed-baseline pattern) is the template; the RB feature builder (`features/rb.py`) already exists; the edge-study harness + persisted universe cache make re-evaluation cheap (`scripts/_dfs_per_position_analysis.py`). Own brainstorm → spec → plan → execute on a feature branch. **Caveats:** DFS stays deprioritized vs the draft work (#52 blocker); WR's edge over *Sleeper-alone* is necessary-not-sufficient for real DFS profitability; and a single position lift may not be enough to flip the pool (QB/TE still drag) — measure, don't assume. **Speedup prereq worth doing first:** vectorize the row-wise `expected_points(...).apply(axis=1)` in `emit_weekly_projections` (the ~75-min single-process walk-forward emit is the iteration bottleneck).
...
- **Hero-eval path (`build_draft_basis`) reconciliation + Test 11 re-run — ✅ DONE (Test 15).** `build_draft_basis` now reconciles placeholder gsis (id_map threaded via `load_inputs`, `full_name` attached from the consensus frame). Re-pulled `external_projections` + `espn_weekly_projections` for 2021–2025 (both were deleted) and re-ran the real-outcome hero eval with `seat_aware` added. **Result: the projected metric and real outcomes DIVERGE.** On real outcomes `now_or_never_floored` wins regular-season WIN% (72.3 % pooled, confirming the original Test 11 on clean data), while `season_value_timing`/`seat_aware` win CHAMP% (~34 %). So the projected-metric goal winner (`seat_aware`) is a championship-ceiling pick, not a win-maximizer, on real outcomes. **CI-tightened at N=25 (2000 paired cells/strategy, `scripts/_hero_pool.py`): the divergence is NOT noise** — `now_or_never_floored` is CI-separated #1 on WIN% (every other strategy −1.66…−3.72, CI excludes 0), and `seat_aware`/`season_value_timing` are CI-separated #1 on CHAMP% (+4.05/+3.90 vs the floor). Open: a better human-opponent model than noisy-ADP bots (TODO #46) is still the top realism lever.
- **Root-cause fix in the ingest crosswalk.** The reconciliation is a consumer-side guard; the deeper fix is that the external-projection ingest should assign real gsis at the source (the raw snapshots assigned placeholders to ~everyone for past seasons). Needs re-pull of ESPN/Sleeper 2021–2025 + a look at why the historical espn/sleeper→id_map crosswalk missed. The guard stays valuable regardless.
- **Per-seat Pareto frontier — RESOLVED by `seat_aware` (shipped).** Post-fix, `season_value_timing` wins wing/mid and `season_value_var` wins the turn (anti-correlated). No *fixed* tilt strictly beat both endpoints (durability/`sv_var_timing` interpolate). `SeatAwareStrategy` routes by the hero's known slot and **strictly beats every baseline on the pooled 6-year win%, CI-separated** (vs sv_timing +0.0038 [+0.0011,+0.0065]; vs sv_var +0.0047 [+0.0002,+0.0091]) — the goal's headline metric, met (Test 14). Open refinement: a *fixed* mechanism that strictly dominates at every seat (e.g. a timing term that correctly vanishes at the turn) would beat the strict per-seat bar `seat_aware` only ties; nice-to-have, not required.
- **Recommendation:** for 16-team half-PPR, **`seat_aware`** (pooled #1, ≥ best at every seat); `season_value_timing` if a single fixed strategy is wanted. Still no *committed* default — the season decision is months out — but `seat_aware` is the data-backed front-runner.

### 50. Correlated-duration injury model for the availability layer — LOGGED, not convinced of value yet (user, 2026-06-19)

**Context.** Surfaced while reviewing the projected-H2H bake-off (Test 12, `reports/draft_strategy_tests.md`). The H2H eval **does** randomize injury/availability — `team_weekly_points` (`league_projection.py`) draws a per-`(sim, week, player)` injury Bernoulli on `p` (historical games-played frac, clamped [0.40, 0.97]; `availability.build_availability`) + byes + the performance-variance model. **The limitation:** injuries are **independent week-to-week**, so a player with `p=0.82` misses ~3 *scattered* weeks — the model gets *average* games-missed right but misses the **lumpiness and the season-ending tail** (ACL → done for year; hamstring → out wks 5–8), which is the injury outcome that actually decides fantasy seasons and the thing a scarcity-floor / handcuff strategy hedges against. Also no in-season waiver replacement; `p` is pure frequency (no injury news/age/position-type).

**Hypothesis worth the slice (if ever built):** a correlated-duration model (season-ending Bernoulli + variable-length absence blocks + optional waiver-replacement) is likely **what lets the projected metric "see" the value of `now_or_never_floored`** — Test 12 showed the floor looks *worse* on the projected metric (scattered injuries can't reward robustness) yet *won* the real-outcome Test 11. So this model would reconcile the projected-vs-actual divergence. Clean upgrade to `availability.py` (+ the season-value MC consumer). **User verdict 2026-06-19: not convinced of its value yet — leave as a logged TODO, do NOT build.** Revisit only if the projected/actual divergence becomes decision-relevant.

### 55. Pre-existing broken test on `main`: `test_generate_preset_vorp_tables::test_main_writes_per_season_tables_and_configs`

`gp.main()` reads `id_map.parquet` from `args.data_root/raw/` (added with the gsis-reconcile step), but the test passes `--data-root tmp_path` and only monkeypatches the `external_projections` read — so it raises `FileNotFoundError: tmp_path/raw/id_map.parquet`. Fails on `main` (identical code); surfaced during the balanced-value branch's full-suite run. **Fix:** the test should write a synthetic `id_map.parquet` into `tmp_path/raw/` (or monkeypatch the id_map read). One of three pre-existing full-suite failures (with `test_backtest_smoke_one_cell` #40 and the `test_draft_board_smoke` AppTest parallel-timeout flake).
...
**Update (2026-07-14 LATEST, branch `feat/auction-robust-win-hero`):** The "hero can't beat the bots" premise was a **seat-1 artifact** — `resolve_bids` broke ties by lowest seat index, dumping every $1 tie on seat 0 (~4 junk scrubs/draft, ~120 pts, ~0.10 win%). **FIXED** (random tie-break via engine rng). Also added `balanced_flat` (`BalancedValueBid(non_increasing_cap=True)`) but the cap-inflation fix is a **WASH** vs the inflating control. **Seat-symmetric truth** (re-swept 12 seats, both markets): `balanced` **BEATS the field in the realistic ESPN market (~0.554 seat-avg, >0.50)** and is **~even in the model market (~0.495)**; it's the win% leader (≥ `patient_deep` nearly everywhere). All prior seat-1 numbers (Runs A–K) were biased LOW → read as pre-fix. Goal (highest reg-win% both markets) → answered: `balanced` already does it in the realistic market. **Next = Slice 2 nomination poisoning** (the hero has zero nomination control today — `simulation.py::_sample_nominee` auto-picks). See `reports/auction_tournament_validation_2026.md` (Run K + seat FIX); memory `auction-bid-model-investigation-status`; spec/plan `docs/superpowers/{specs,plans}/2026-07-14-auction-robust-win-hero*`.

**Update (2026-07-14, branch `feat/auction-balanced-value` → PR):** Slice 1 shipped — new `balanced` contestant (`BalancedValueBid`: premium-over-fair + per-player pace cap, **no `_budget_urgency`**), the tenth `_MODELS` model; existing strategies untouched. **Run I:** `balanced` is the top hero in BOTH markets — model-priced playoff 0.46 (CI-sep above `vorpshare`), ESPN-anchored playoff 0.24 (beats `patient_deep` CI-sep on champ+bye). Full superpowers-go (spec/plan `2026-07-13-auction-balanced-value*`). No winner — Sept decision. **Next: Slice 2 = nomination warfare.** See PM 2026-07-14.
...
**Update (2026-06-18, branch `feat/auction-budget-urgency`, PR #81):** budget-urgency slice shipped — shared `_budget_urgency` late-draft deployment factor (`URGENCY_GAIN=3.0`) applied at a single exit by all 7 contestants (empty-roster behavior unchanged) + new 8th contestant `StudsAndDepthBid`. **Run F** (eight-model bake-off vs the realistic market, 16-team, 40 seeds) recorded: urgency compresses the field by forcing cash-hoarders to deploy (`inflation` playoff 0.03→0.12, `marginal` 0.00→0.08, `vorpshare` 0.05→0.13 vs Run E); `studsdepth` mid-pack, `anchors` last. **Run G** (same knobs, **12-team**) recorded: the shallower field *spreads* instead of compressing — `vorpshare` (playoff 0.44) and `patient` (0.39) break away from the pack; hero far more competitive (best 0.44 vs 0.50 baseline) but still sub-baseline; `anchors` last, `studsdepth` mid-pack. VORP table rebuilt from the ESPN+Sleeper consensus re-scored to half-PPR (`data/vorp_2026/half_12team.parquet`, untracked). **Still no winner — September decision unchanged.** Better-calibrated bot WTP (anchored on real published auction values) remains the biggest open realism lever.

**Two follow-up directions (user, 2026-06-18; see `reports/auction_tournament_validation_2026.md` Run G note):**
- **(49a) Multi-year averaging.** Year-to-year results swing wildly (Runs A–G are one 2026 snapshot each). Re-run the auction bake-off across every season we have projection data for and average the per-model metrics, so the ranking is a multi-year mean, not one noisy draw. Reliability lever; do before declaring any September winner. **Data-ingest prereq SHIPPED (2026-06-19, branch `feat/multi-year-auction-ingest`, spec/plan `docs/superpowers/specs|plans/2026-06-19-multi-year-auction-ingest.*`):** preset generation is now season-aware (`presets.get_preset(s, n, season=2026)` → `data/vorp_{season}/`; `scripts/generate_preset_vorp_tables.py --season Y`; new `scripts/refresh_external_seasons.py` wrapper loops ingest+gen). Ingested `external_projections` for 2021–2025 and generated per-season preset VORP tables **+ `.league.json` configs** for **2021–2026** (all 3 scoring × 3 sizes), each carrying `espn_auction_dollars`. R6 gate passed — half_12team priced-player counts: **2021=222, 2022=266, 2023=279, 2024=230, 2025=160, 2026=201** (all `has_usable_espn_prices`). Auction-value basis varies by season (pre-2023 **crowd-only**; 2025 **expert-only**; 2023/24/26 both) and half-PPR anchors on the PPR expert column in expert-only seasons — documented caveats, not corrected. **Multi-year averaging RUN (2026-06-19, branch `feat/patient-deep-contestant`):** bake-off across all 6 seasons (12-team half-PPR, ESPN bots, 20 seeds × 300 sims, seat 6, 9 contestants incl. the new standing `patient_deep` = `PatientValueBid(scrub_frac=0)`). **NOTE — corrected after a bot-pricing fix (see #49c below):** with the fix, **`patient_deep` dominates** — mean playoff **0.825 (+0.25 over #2** `patient` 0.577), champ 0.164, **sd 0.020**, **#1 in all 6 seasons** (0.81–0.86). The pre-fix run had `inflation`/`static` at an apparent #2/#3 with a spurious "era-shift" — that was the flat-$1 artifact; corrected they're ~0.50 (#4/#5). Full table + correction note in `reports/auction_tournament_validation_2026.md` + PM. **September candidate: adopt `patient_deep` / re-tune `patient` to `scrub_frac≈0`.** Untested: sweep the 0.4 unranked-discount; 16-team auction; higher seeds; pooled aggregation. New reusable tool: `scripts/inspect_auction_drafts.py` (best/worst-draft roster inspector). Artifacts untracked (regen via `refresh_external_seasons.py` + `generate_preset_vorp_tables.py --season 2026`).

  **(49c continued — bot-pricing methodology fix, 2026-06-19, same branch):** `espn_anchored_bot_prices` valued every ESPN-*unranked* player at a flat `min_bid`, so bots rostered random $1 scrubs (started a 3rd-string QB — caught via the inspector). Fixed: unranked players fall back to `_UNRANKED_MODEL_DISCOUNT` (0.4) × our VORP-based model value (cheap but ordered). Bots now field real depth; preserves the hero edge (exploited rookies are ESPN-priced-cheap, not unranked). Changed all ESPN-anchored bake-off numbers (the flat-$1 field was inflating the stud-buying heroes). ~~Open: sweep the 0.4 discount.~~ **SUPERSEDED — broke-bot snake-draft behavior SHIPPED (2026-06-19, branch `feat/auction-broke-bot-snake`, spec/plan `docs/superpowers/specs|plans/2026-06-19-auction-broke-bot-snake.*`).** The discount was a stopgap that bent *flush*-bot pricing to fix a *broke*-bot behavior bug; the real fix targets the behavior. Now a bot at the `feasible_max == min_bid` floor drafts like a **snake drafter**: a per-bot fixed noisy-ADP `SnakeBoard` (drawn once per draft from a dedicated CRN substream, `adp_jitter=8.0`) picks its single best-available-for-needs target; as nominator it nominates that target, as responder it bids $1 only on it (else abstains), backed by a new "nominator takes its nominee at $1 if unbid" engine backstop. Flush bots / hero / flush-hero nomination unchanged (gated on `adp_ok and not forced`; disabled + byte-identical to `main` when the pool has no usable `consensus_adp`). Reuses the snake-draft `bot_pick` core (extracted `_best_by_noisy_adp`). The `_UNRANKED_MODEL_DISCOUNT` (0.4) knob is **retained** (still prices unranked players for *flush* bots) but its sweep is dropped. New: `src/projections/draft/assistant/auction/snake_bot.py`. **Validation DONE — multi-year bake-off RE-RUN with the snake-draft field (2026-06-19, recorded in `reports/auction_tournament_validation_2026.md` "Multi-year bake-off RE-RUN"):** identical knobs to the discount-era #49a run (12-team half-PPR, ESPN bots, 20 seeds × 300 sims, seat 6, 9 contestants). **`patient_deep` still #1 in all 6 seasons** (mean playoff **0.660**, champ 0.112), but the snake-draft field is harder — *every* model's playoff% fell (mean Δ ≈ −0.08) and the mid-pack re-ranked: shipped `patient` fell #2→#5 (0.577→0.408) and `marginal` cratered (−0.205), since both relied on the old scrub-dumping field for free depth. `patient_deep`'s lead compressed (+0.25→+0.18) but the `patient_deep > patient` gap held/strengthened (+0.252 mean). The **snake-draft field is the new methodology baseline**; no default change (September decision unchanged). Pre-existing unrelated full-suite failure: `tests/backtest/test_backtest_smoke.py` (stale on-disk `data/features/wr` missing `preseason_implied_team_total` → `scripts/refresh_features.py wr`).
- **(49b) RL bid-policy agent.** Hand-authored bid models all cluster at/below the field's fair share — evidence we're not searching the strategy space well by hand. Build a reinforcement-learning agent that learns a bidding policy against the bot market (then self-play). Larger new slice — needs its own spec/plan (brainstorm → spec → plan → execute on a feature branch).
- **(49c) Real-auction-value bot anchor — break the shared-value problem.** The deeper realism lever: the bots currently price off the hero's *own* `auction_dollars` ± noise (no exploitable bias). **Slice 1 SHIPPED** (branch `feat/espn-auction-values-ingest`, PR #82): ingest + plumb real ESPN auction values (crowd `auctionValueAverage` now / expert `draftRanksByRankType` historical; Sleeper has none) to an Optional `espn_auction_dollars` pool column; no behavior change. **Slice 2 — code SHIPPED** (branch `feat/auction-bot-real-price-anchor`, spec/plan `docs/superpowers/specs|plans/2026-06-18-auction-bot-espn-anchor.*`): the **bots** now price off real ESPN values while the **hero** stays on our SOS model — the hero's edge is the gap between them. Design decisions (brainstorm 2026-06-18): **ESPN-only with a $1 floor for unpriced players** (choice B, "max divergence" — bots ignore the deep pool ESPN didn't rank, so the SOS hero finds bargains there); **global budget-normalization** preserving ESPN's cross-position bias. Mechanism: a shared `_allocate_surplus` helper (hoisted from `generate_auction_values`) + `espn_anchored_bot_prices` (SOS allocation over `espn_auction_dollars`, sums to budget); an engine-internal `bot_dollars` column the bots read (hero + nomination stay on `auction_dollars` → clean one-variable A/B); CLI flag `--bot-prices {espn,model}` **default `espn`** with graceful warn→model fallback when the pool has no ESPN data; + a CLI ESPN-vs-ours `value_delta` diagnostic. **Known property:** deep leagues (16-team) concentrate the surplus on priced studs (inflation) — observed in Run H, not guarded. **Slice 2 Run H bake-off — DONE (2026-06-19).** Fresh `external_projections` re-ingest (asof 2026-06-19, 199 crowd / 160 expert ESPN auction values) → regenerated all 9 preset tables with `espn_auction_dollars` populated; bake-off at half-PPR 12- and 16-team, seat 6, 40 seeds, n_sims=500, realistic mixed field, as a clean `--bot-prices espn` vs `model` A/B (recorded as **Run H** in `reports/auction_tournament_validation_2026.md`). **Result (data, no winner): ESPN-anchored bots are exploitable — the hero's edge appears for the first time in the realistic market.** Switching only the bots' anchor to real ESPN values, most value heroes improve and several clear fair share (16-team field-mean playoff Δ ≈ +0.06: static +0.13, marginal +0.17, patient +0.15, overbid +0.12, studsdepth +0.10; 12-team patient → 0.62, above the 0.50 baseline); `inflation`/`vorpshare` are the exceptions. Diagnostic: ESPN underprices rookies our model rates ($1–8 vs our $21–30) — the bargains the SOS hero grabs. Deep-league inflation observed (bot top $73/12-team, $100/16-team). **Not comparable to Runs F/G levels** (different seat/table/n_sims); the valid comparison is the within-Run-H espn-vs-model A/B. **No winner — September decision unchanged.** **Run H follow-up (2026-06-19, see report):** sweeping `PatientValueBid` found the shipped `scrub_frac=0.50` is badly mis-tuned for 12-team auction — `scrub_frac=0` ("hoard mid-tier breadth") lifts playoff 0.62→0.83 (full test, 40 seeds/n_sims=500, CI-separated +0.205) and makes `patient_deep` the runaway field leader; competing for studs to spend the budget is a wash (breadth is the lever, not budget-deployment). **September action candidate:** re-tune the `patient` default to `scrub_frac≈0` or add a `patient_deep` contestant. 16-team auction untested (12-team is the relevant size for this league). **Slice-3 follow-ups (Open Questions in the spec):** room-driven nomination; an A/B of the ESPN+SOS-backfill variant vs choice B; a per-player inflation cap if Run H shows 16-team studs are unrealistic; fold ESPN-anchored bots into the multi-year averaging (#49a).
...
**Feature-cache staleness RESOLVED (2026-06-24, branch `feat/dfs-projection-edge-study-verdict`).** The on-disk `data/features/{qb,rb,wr,te}` partitions were rebuilt from freshly re-ingested raw (`pbp`/`ngs`/`snap_counts`/`depth_charts`/`schedules`/`draft_picks`) for 2018–2024 — 604 partitions, current schema, Vegas team-context columns genuinely populated (verified non-null). This was the DFS-verdict blocker (#54) and is the likely root cause of the long-standing `test_backtest_smoke_one_cell` failure (stale features missing `preseason_implied_team_total`); re-check that test now that the cache is fresh. **Still open:** the `tests/backtest/model_metrics.json` walk-forward snapshot regen below (a separate artifact — NOT done here). Raw seasons 2016–2017 were also ingested but 2017 features are NOT built (schema `ge=0` blocker on `opp_allowed_*_fppg_l4`, see #54).
...
### 39. Fair weekly start/sit benchmark — does our weekly model beat ESPN weekly? (✅ CLOSED — answered by #54's verdict, 2026-06-24)

**CLOSED 2026-06-24:** #54's verdict answers this — the fair weekly benchmark (re-pointed at Sleeper weekly + DK base, pre-registered gate) returned **STOP**: our weekly model does NOT beat the market proxy (closer to actual only 0.476 of disagreement cells, CI 0.456–0.495). So as an in-season start/sit edge over consensus, the home-grown weekly model has no measured advantage overall (WR is the lone bright spot at 0.535). See #54 and `reports/dfs_projection_edge_2026-06-24.md`. Original below for history.

**Update 2026-06-23:** the DFS Layer 1 edge study (#54) builds exactly this method, re-pointed at **Sleeper weekly** (historically retrievable, unlike ESPN) + **DraftKings** base scoring, with a pre-registered statistical gate. The harness is shipped + unit-tested; the real verdict is deferred on the TODO #40 feature-staleness blocker. This item closes once #54's verdict run lands. Original below.
