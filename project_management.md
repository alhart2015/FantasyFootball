...
## DFS — RB feature lift (#55) VERDICT: **STOP stands for RB** (2026-06-25, branch `feat/rb-feature-signal-probe`)

**Status:** the #54 per-position STOP named RB the highest-leverage lift target. Investigated end-to-end via the `superpowers-go` pipeline (brainstorm → spec → **spec-review** 3 iters to clean → plan → **plan-review** 3 iters to clean → subagent-driven execution). Spec/plan `docs/superpowers/specs|plans/2026-06-25-rb-trajectory-vegas-features.*`. **Verdict: STOP — RB is not liftable with the available signal; integration reverted, RB stays on `baseline`.**

**The chain.** (1) Two cheap model-swap paths were already dead: decomposition NULL'd (#54 follow-up), and a fresh RB-only bake-off (`reports/rb_model_bakeoff_2026-06-25.md`) showed `baseline` (RidgeCV) beats `lightgbm-nb`/`ensemble` on every metric — the **signal-limited** signature (a tree can't beat ridge on the same features), so only *new feature signal* could move RB. (2) RB lacked two feature families WR/QB have. **Probe-first** (`reports/feature_probe_rb_summary.md`): **Vegas NULL**, trajectory **core** (age/is_rookie) **NULL**, trajectory **trend** (`volume_trend_l4_minus_prior_l4`, `snap_pct_change_l4_vs_prior_l4`) **SIGNAL on rushing_yards** (pooled ΔRMSE −0.173, CI [−0.284,−0.060], robust 4/4 years) — this *contradicted* the spec's prior that both families would NULL. (3) Integrated the trend pair (schema + builder + `_RB_FEATURE_COLUMNS`, mirroring WR) → **G2 adoption gate DO_NOT_ADOPT**: the rushing-yards gain **does not survive into composite fantasy points** (probe Phase-2 composite paired ΔRMSE −0.0044, CI [−0.0156,+0.0061] brackets 0; augmented bake-off `reports/rb_model_bakeoff_augmented.md` aug-baseline rmse 6.5630 vs old 6.5661 = −0.003 sub-noise; lightgbm-nb 6.604 + ensemble 6.573 still lose to old baseline → no class flip). Mechanism: the other 5 RB stats NULL'd (diluting the gain) and `BaselineModel.fit`'s `dropna` drops ~40% of training rows for the 40%-NaN trend feature.

**Shipped (kept on branch):** reusable infra — `scripts/backtest.py --features-root/--position` + `scripts/refresh_features.py --features-root` passthroughs (enable non-destructive feature-set A/B) — plus all probe/bake-off evidence reports. **Reverted:** the schema/builder/`_RB_FEATURE_COLUMNS` integration (commits 44bfb5f/4fb2f3e/4bc783c) — no dead columns per spec §6/§8. **Did NOT** touch the production `data/features/rb` cache, flip the dispatch default, or re-run the ~75-min edge study (the model-backtest gate already settled it; the edge study would only confirm a non-change).

**Process note:** the integration exposed a real coupling the plan over-split — a `nullable=True` schema column is *required-present*, so `RbFeaturesSchema` and `build_rb_features` must change together or validation breaks; and adding a trend feature without prior-season test-fixture history empties `BaselineModel.fit`'s training set via `dropna` (fixed by extending `baseline_weekly_stats_rb`, mirroring PR #26). The per-task diff-review missed the `test_baseline_rb` regression (test not in the diff); a full-suite run caught it.

**Open (non-gating, logged not pursued — DFS deprioritized vs #52):** the trend feature's rushing-yards signal is real but the box-score composite baseline can't monetize it. A **rushing-yards-only or usage-decomposed RB model** (Option C from the brainstorm, not the box-score baseline) might capture it; revisit only if DFS is reprioritized.
