...
- Consensus blend (simple average first; accuracy-weighting later) → published preseason projection per player. **Cross-source rookie-matching caveat:** each source placeholders a rookie by its OWN native id (so an ESPN-rookie and the same Sleeper-rookie get DIFFERENT `99-` placeholders) — the blend must match rookies on (name, position) across sources until real gsis_ids propagate, not on the placeholder gsis.
