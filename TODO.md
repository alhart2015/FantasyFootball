...
- **Hero-eval path (`build_draft_basis`) — deferred reconciliation.** Test 11 (real-outcome hero-vs-bots, 2021–2025) loads its pool via `build_draft_basis`, whose table lacks a `full_name` column (the reconcile key), so it was **not** fixed in this pass. Test 11's availability was therefore also degraded (position-average `p`, no byes). Thread `full_name` (from the consensus frame) into `build_draft_basis` and reconcile there, then **re-run Test 11** — its "now_or_never_floored most consistent" finding may also shift now that availability is real (the projected Test 14 now favors the season_value family; whether the *real-outcome* eval agrees is the open question). Heavy run (BSOD-prone), hence deferred.
