...
Decision rule encoded in the CSV `decision` column. We compare per-class
*coverage gap* (target=0.80 minus empirical coverage) -- the question the
Phase 0 gate actually asks is "which class is more miscalibrated", not
"which class has more variance." Variance share alone is misleading
because raw Var(actual) is dominated by yards even when yards are
well-calibrated.
  - count_coverage_gap > yards_coverage_gap + 0.02 -> "proceed_phase_1"
  - yards_coverage_gap > count_coverage_gap + 0.02 -> "stop_file_yards_plan"
