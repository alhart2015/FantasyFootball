...
- Add 1–2 scraped preseason sources (FantasyPros/CBS/NumberFire) for a real multi-source **points** consensus (user OK'd scraping). This is the next slice — it's what makes `projected_points_ppr` a ≥2-source average instead of single-source ESPN.
...
- **`pd.concat` `FutureWarning` at `external_projections.py:398`** (surfaced running the live 2026 snapshot, 2026-06-09): `pd.concat(frames, ...)` warns because Sleeper rows carry all-NA stat columns, and a future pandas will stop excluding all-NA columns when inferring result dtypes — risking silent dtype drift on the stat-line columns the consensus reads. Harmless today; make explicit (set stat-column dtypes before concat, or concat only the populated ESPN frame).
- **DRY: consolidate `STAT_FIELDS`** — the 9-field preseason stat-line tuple is duplicated in `ingest/external_projections.py` and `consensus/blend.py`; they must stay in sync. Promote to a single shared constant (natural home: `schemas.py`, next to `ExternalProjectionSchema`/`ConsensusProjectionSchema`) that both import. (Flagged in the blend code review; deferred to avoid cross-task scope creep.)
