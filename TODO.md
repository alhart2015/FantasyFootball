...
- **(49c) Real-auction-value bot anchor — break the shared-value problem.** The deeper realism lever: the bots currently price off the hero's *own* `auction_dollars` ± noise (no exploitable bias). **Slice 1 SHIPPED** (branch `feat/espn-auction-values-ingest`, spec/plan `docs/superpowers/specs|plans/2026-06-18-espn-auction-values-ingest.*`): ingest + plumb real ESPN auction values (crowd `auctionValueAverage` now / expert `draftRanksByRankType` historical; Sleeper has none) to an Optional `espn_auction_dollars` pool column; no behavior change. **Slice 2 (next):** feed `espn_auction_dollars` into `generate_auction_values` as `reference_prices`, point the **bot** WTP at it while the **hero** keeps our model, decide NA-fallback for unpriced players, rescale ESPN $ to league budget × teams, run the bake-off. Prereq: a fresh `external_projections` re-ingest to populate the column.
