...
| Date | Decision | Rationale |
|---|---|---|
| 2026-05-11 | `weekly_stats` ingest fetches `stats_player/stats_player_week_<season>.parquet` directly (TODO #32 option 1) | `nfl_data_py` still hits the legacy `player_stats_<season>.parquet` URL which 404s for ≥ 2025. Direct `pd.read_parquet` over the new nflverse release URL adds no new dep, uses a single code path for 2018-2025, and the rename layer (`passing_interceptions → interceptions`, `sacks_suffered → sacks`) plus the kept legacy `recent_team → team` map make the synthetic `fake_weekly_df` fixtures continue to validate. Option 3 (full nflreadpy migration) still owed before the 2026 season opens. |
