...
- **(49a) Multi-year averaging.** Year-to-year results swing wildly (Runs A–G are one 2026 snapshot each). Re-run the auction bake-off across every season we have projection data for and average the per-model metrics, so the ranking is a multi-year mean, not one noisy draw. Reliability lever; do before declaring any September winner. **Data-ingest prereq SHIPPED (2026-06-19, branch `feat/multi-year-auction-ingest`, spec/plan `docs/superpowers/specs|plans/2026-06-19-multi-year-auction-ingest.*`):** preset generation is now season-aware (`presets.get_preset(s, n, season=2026)` → `data/vorp_{season}/`; `scripts/generate_preset_vorp_tables.py --season Y`; new `scripts/refresh_external_seasons.py` wrapper loops ingest+gen). Ingested `external_projections` for 2021–2025 and generated per-season preset VORP tables **+ `.league.json` configs** for **2021–2026** (all 3 scoring × 3 sizes), each carrying `espn_auction_dollars`. R6 gate passed — half_12team priced-player counts: **2021=222, 2022=266, 2023=279, 2024=230, 2025=160, 2026=201** (all `has_usable_espn_prices`). Auction-value basis varies by season (pre-2023 **crowd-only**; 2025 **expert-only**; 2023/24/26 both) and half-PPR anchors on the PPR expert column in expert-only seasons — documented caveats, not corrected. **Multi-year averaging RUN (2026-06-19, branch `feat/patient-deep-contestant`):** bake-off across all 6 seasons (12-team half-PPR, ESPN bots, 20 seeds × 300 sims, seat 6, 9 contestants incl. the new standing `patient_deep` = `PatientValueBid(scrub_frac=0)`). **NOTE — corrected after a bot-pricing fix (see #49c below):** with the fix, **`patient_deep` dominates** — mean playoff **0.825 (+0.25 over #2** `patient` 0.577), champ 0.164, **sd 0.020**, **#1 in all 6 seasons** (0.81–0.86). The pre-fix run had `inflation`/`static` at an apparent #2/#3 with a spurious "era-shift" — that was the flat-$1 artifact; corrected they're ~0.50 (#4/#5). Full table + correction note in `reports/auction_tournament_validation_2026.md` + PM. **September candidate: adopt `patient_deep` / re-tune `patient` to `scrub_frac≈0`.** Untested: sweep the 0.4 unranked-discount; 16-team auction; higher seeds; pooled aggregation. New reusable tool: `scripts/inspect_auction_drafts.py` (best/worst-draft roster inspector). Artifacts untracked (regen via `refresh_external_seasons.py` + `generate_preset_vorp_tables.py --season 2026`).
  ...
  **(49c continued — bot-pricing methodology fix, 2026-06-19, same branch):** `espn_anchored_bot_prices` valued every ESPN-*unranked* player at a flat `min_bid`, so bots rostered random $1 scrubs (started a 3rd-string QB — caught via the inspector). Fixed: unranked players fall back to `_UNRANKED_MODEL_DISCOUNT` (0.4) × our VORP-based model value (cheap but ordered). Bots now field real depth; preserves the hero edge (exploited rookies are ESPN-priced-cheap, not unranked). Changed all ESPN-anchored bake-off numbers (the flat-$1 field was inflating the stud-buying heroes). Open: sweep the 0.4 discount.
