...
- **Beat the per-seat Pareto frontier (the original goal, unmet).** Post-fix, `season_value_timing` wins wing/mid and `season_value_var` wins the turn (anti-correlated; timing helps long waits, hurts back-to-back picks). No hand-authored tilt (durability `−μ(1−p_week)vorp`; `sv_var_timing`) strictly beats both endpoints at every seat — they interpolate the frontier. A genuinely better mechanism (e.g. a timing layer that correctly vanishes at the turn, or a non-myopic lookahead over the sv-marginal) is needed to strictly dominate. Search harness: `scripts/_snake_knob_search.py` (train 2021/23/25 → holdout 2022/24/26, CI). Deferred (user: bank findings, ship the fix).
- **Recommendation pending the above:** for 16-team half-PPR the data favors `season_value_timing` (pooled #1) / `season_value_var` (turn). No committed default — the season decision is still months out.
