...
**Gates:** `tests/test_draft/` green (479 passed / 1 network-skip in the final review); auction subsystem no-regression; mypy clean (332 files); ruff check + format clean. **Remaining (Run H, data-gathering):** re-ingest `external_projections` (2026) + regen preset tables, bake off the 8-model field vs the ESPN-anchored bots at half-PPR 12- and 16-team, write Run H vs Runs G/F — **no winner declared (September decision unchanged).** **Env note:** a subagent ran `pip install -e .` in the shared `.venv` pointing it at this worktree; re-install from the main checkout when done. **Slice-3 follow-ups:** room-driven nomination; ESPN+SOS-backfill-vs-choice-B A/B; inflation cap; multi-year averaging (#49a).
