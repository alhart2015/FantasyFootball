"""Cross-position smoke for LightGBMNbModel — Plan 5c.

Single fit + predict for each of the 4 positions on synthetic fixtures, driven
by `POSITION_DISPATCH[position].factories["lightgbm-nb"]`. Catches regressions
...
def test_lightgbm_fit_predict_smoke(position: Position) -> None:
    """Each position's LightGBMNbModel can fit and predict via POSITION_DISPATCH."""
    features, weekly_stats = _build_synthetic_data(position)
    factory = POSITION_DISPATCH[position].factories["lightgbm-nb"]
    model = factory()
