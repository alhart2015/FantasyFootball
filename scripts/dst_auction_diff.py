...
cfg_with = build_league_config(json.load(open(ROOT / "data/leagues/critts_2025_2026/espn_raw.json")))
...
refresh_dst_projections(tmp, season=2026, asof=date(2026, 9, 6), payload=fetch_espn(2026, limit=1500))
...
for gsis, g in dst_raw.groupby("gsis_id"):
    ...
    dst_rows.append(
        {
            "gsis_id": gsis, "season": 2026, "position": "DST", "ruleset": ruleset.name,
            "n_weeks": 17, "season_mean": pts, "season_p10": 0.0, "season_p50": 0.0,
            "season_p90": 0.0, "model_id": "espn_dst_2026", "generated_at": GEN_AT,
        }
    )
...
print("=" * 72)
print(f"POOL:  {cfg_without.total_pool_size}  ->  {cfg_with.total_pool_size}"
...
for _, r in cmp.sort_values("before", ascending=False).head(15).iterrows():
    print(f"{str(r['name'])[:23]:<24}{r['pos']:>4}{r['before']:>8.0f}{r['after']:>7.0f}"
          f"{r['delta']:>+7.0f}")
...
for pos, g in cmp[cmp["before"] > 0].groupby("pos"):
    print(f"{pos:<5}{len(g):>5}{g['before'].mean():>13.1f}{g['after'].mean():>12.1f}"
...
for g, r in list(dst.iterrows())[:6]:
    print(f"{names.get(g, g):<14}{r['season_mean_fpts']:>8.1f}{r['vorp']:>8.1f}"
