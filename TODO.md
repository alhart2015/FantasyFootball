...
**Update 2026-07-13 (branch `feat/auction-budget-urgency` → PR to `main`):** The full bid-model stack (stars-and-scrubs + realistic market + budget-urgency, Runs C–F) is merged to `main` via PR; no winner (Sept decision holds). **Breakthrough this session:** an all-bot diagnostic + a `BalancedValueBid` prototype show the winning policy is *balanced breadth* (small premium over fair value + a per-player pace cap ≈2× the even per-slot share), NOT stars-and-scrubs — `BalancedValueBid` is the first hero to cross the playoff baseline (12-team: playoff 57% / champ 6.6% vs best-old-hero `vorpshare` 45%/5%), and this branch's `_budget_urgency` ramp is self-harm (toggling it ON craters the strategy). **Next (new branch):** Slice 1 = ship `BalancedValueBid` + make `_budget_urgency` opt-out; Slice 2 = nomination warfare (hero-controlled nominations) to beat the balanced bots. Use 12-team half-PPR (user's real auction size). See PM 2026-07-13.
