...
- Consensus blend (simple average first; accuracy-weighting later) → published preseason projection per player. **Cross-source rookie-matching (changed in PR #54):** rookie placeholders are now hashed from normalized **(name, position)** (accent-folded, suffix-stripped), so the same rookie already gets the SAME `99-` placeholder across ESPN and Sleeper and joins on `gsis_id` at ingest. The consensus blend should *refine* this — the simple key still misses spelling/nickname/hyphen divergence (e.g. "Amon-Ra", nicknames) and can collide two distinct same-name+position players (logged, not prevented). Promote `_placeholder_name_key` to a shared identity util the blend and ingest both use, so they agree by construction rather than re-deriving the match.
