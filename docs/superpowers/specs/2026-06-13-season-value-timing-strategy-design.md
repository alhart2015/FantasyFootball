...
2. **Timing layer (no new MC):** `next_pick = my_next_pick(state.current_pick, state.my_slot, state.n_teams, state.rounds)`.
   ...
   - **Otherwise:** per pruned candidate, survival prob `p = survival.p_available(adp, next_pick)` with null ADP → `p = 1.0` (nn's convention). Compute, per position, the **expected best surviving marginal** via the shared helper (§4):
     `opp_cost[pos] = E[ max marginal among pos candidates that survive to next_pick ]`.
3. **Score:** `score(c) = marginal(c) − opp_cost[pos(c)]`. Pruned-out candidates keep `marginal = 0` (so their score is `−opp_cost[pos]`) — cosmetic tail; the recommended pick (argmax) is always an evaluated candidate, exactly as in `season_value`.
...
- `seat_layout(seed)` and `collect_results(...)` take the two strategy implementations for roles **A** and **B** (default `A = now_or_never`, `B = season_value`). `seat_layout` keeps the same seat sets (A at `{2,6,10,14}`, B at `{4,8,12,16}`, swapped on the paired even seed; bots at the odd seats). Strategy **labels** in `LeagueResult`/`BacktestResult` follow the role's strategy name (so the output table reads `season_value_timing` / `season_value` / `bot` when testing this strategy).
...
- The CLI (`scripts/h2h_backtest.py` / `h2h_backtest_chunked.py`) gains flags to pick the A/B strategies (default unchanged). `season_value_timing` requires `--sigma` (survival) in addition to the existing `--season`/`--strategy-n-sims`.
- The 16-team `seat_layout` guard from the `/code-review` pass (TODO-adjacent) stays; nothing here changes the team-count assumption.
