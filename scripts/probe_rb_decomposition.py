...
from projections.scoring.score_distribution import _scoring_coefficients
...
_COEFFICIENTS: dict[Stat, float] = _scoring_coefficients(_RULESET)
...
def _factor_orthogonality_table(output: WalkForwardOutput) -> list[str]:
    ...
    Zero-std guard mirrors target_decomposition_probe._factor_residual_correlation:
    np.corrcoef divides by std and emits a RuntimeWarning + nan on zero-variance
    inputs; intercept that explicitly.
    """
    lines: list[str] = [
    ...
    for stat in _RB_DECOMPS:
        ...
        for entry in per_year_list:
            ...
            if vol.size < 2 or float(vol.std()) == 0.0 or float(eff.std()) == 0.0:
                rhos[entry.eval_year] = float("nan")
                continue
            rho_matrix = np.corrcoef(vol, eff)
            rhos[entry.eval_year] = float(rho_matrix[0, 1])
...
def _stat_to_fpts(stat: Stat, delta: float) -> float:
    """Translate per-stat delta to composite-fpts delta via the scoring layer.

    Honors the CLAUDE.md scoring-layer rule: scoring math lives only in
    src/projections/scoring/. We reuse _scoring_coefficients(ruleset), which
    returns a Stat-keyed dict of per-stat coefficients with the canonical
    convention (1/yds_per_pt for yards stats, direct multiplier for TDs and
    receptions).
    """
    return delta * _COEFFICIENTS[stat]


def _coverage_section(*, header: str, by_year: dict[int, float], threshold: float) -> list[str]:
    """Render one per-eval-year coverage list under a `### header`."""
    lines = [f"### {header}"]
...
def _write_summary(
    ...
    *,
    ...
    for v in verdicts:
        fpts_delta = _stat_to_fpts(v.stat, v.rmse_delta.point)
    ...
    lines.extend(
        _coverage_section(
            header="Carries > 0 rate (rushing axis)",
            by_year=output.coverage_carries_by_year,
    ...
    lines.extend(
        _coverage_section(
            header="Targets > 0 rate (receiving axis)",
            by_year=output.coverage_targets_by_year,
            threshold=coverage_threshold,
        )
    )
    lines.append("")
        ...
        rows.append(
            {
                ...
                "composite_fpts_equivalent": _stat_to_fpts(v.stat, v.rmse_delta.point),
                "verdict": v.verdict,
            }
        )
    path.parent.mkdir(parents=True, exist_ok=True)
