...
**Mechanism finding (the load-bearing insight):** `p90` and `blend_70_30` produce **byte-identical rankings** to `mean` across every cell, because per-player σ scales roughly proportionally to mean — so `p90 = mean + k·σ` is a monotonic transform of `mean` and preserves rank order. The hypothesis that "the upper tail captures elite signal mean ranking discards" is FALSIFIED in the concrete cases that motivated TODO #33: Chase 2024 mean=250.74, **p90=283.80**, actual=403; Gibbs 2024 mean=258.75, p90=295.29, actual=390.40; Henry 2024 mean=275.64, p90=311.38, actual=381.40. The whole distribution is shifted-down for elites, not just its mean.
