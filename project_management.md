...
**Two things had to be ingested that never had been.** We pull rosters and have never pulled the players *nobody* rosters — `fetch_free_agents` asks the league endpoint for `kona_player_info` filtered server-side to FREEAGENT and WAIVERS. And `parse_rosters` read ESPN's `injuryStatus` and threw it away, which meant every projection in the repo treated a player on IR as fully fit. `InjuryStatus` now lives in `schemas.py` with `parse_injury_status` as its only sanctioned constructor.

**The question is not "is he better than my worst player".** In a 16-team league with seven starters and five bench spots, 192 players are rostered, the wire is thin, and that question is nearly always yes and nearly always irrelevant. The real move is a stream, and a stream is worth making only if it changes who you START — so stage 1 is `best_lineup(roster + him) − best_lineup(roster)`. A genuinely good free agent who would not crack the lineup scores exactly 0.0, which is the truthful answer most weeks. Byes, FLEX and positional scarcity all fall out of it with no rule about any of them.

**The objective is change in expected season wins, because points across two horizons are not a common currency.** "20% better next week" and "2.5 expected wins worse for the year" cannot be compared by eye. Expected wins IS the conversion — 20% for one week is 0.2 wins — so `--wins` re-simulates the shortlist and reports one signed number. Only the shortlist: each is a full Monte-Carlo season, and stage 1 exists to make sure it runs on candidates that could possibly matter.

**The injury constants are measured.** `scripts/measure_injury_impact.py` joins five seasons of weekly injury reports to ESPN weekly projections and to `weekly_stats` scored under the league ruleset. Three findings, each of which changed a decision:

- **Any aggregate over injury designations must condition on projected volume, or it measures roster churn.** "54% of Questionable players play" is true and useless. Split by what ESPN projected him for: 11% under two points, **99.4% at 10-15, 100% at 15+**. A Questionable *starter* plays. The unconditioned number was about to become a half-game penalty on exactly the players it does not apply to.
- **He still delivers ~14% under his projection** (83.4% against a healthy 96.5%, n=846), so the tag is informative even when it does not keep him off the field. Multiplier 0.86.
- **ESPN zeroes `Out` in its weekly feed and verifiably does NOT discount `Questionable`** (his projection is 100.4% of his own healthy-week median, n=843). So the 0.86 is ours to apply, and anything applying an `Out` multiplier on top of that feed double-counts. That guard has a flag and a test.

**A Questionable tag covers one week, and that reshaped the design.** Over ten remaining weeks it is a ~1.4% haircut that will never flip a drop/add; on a single week it is a 14% cut, which is the size that decides a close start/sit. Two horizons, two adjustments, both built — collapsing them into one constant was the original mistake.

**IR is the one guess left, and the one that matters most.** It is a roster designation rather than a game status, appears in no injury report, and cannot be measured the way the others were. Set to the NFL minimum of four games, which biases toward *keeping* the player — dropping someone who returns in Week 12 is the worse mistake. The compensation is `ingest/injury_news.py`: ESPN's **public** site API (no cookies, same athlete id) carries the beat-reporter write-up with body part, severity, a date, and the sentence that usually states a timeline. **Shown, never parsed** — a regex for "four to six weeks" cannot tell games from practices, and the error would land inside a projection where nobody can see it.

**`scripts/measure_swap_noise.py` is a gate, not a diagnostic.** Reporting Δ wins is only worth doing if the change beats the error on it. Measured at 2,000 sims: one estimate carries sd 0.090 wins, an unpaired difference ~0.127, a paired one 0.062. **Pairing is worth 2x, not the order of magnitude the spec first claimed** — `project_league_standings` reseeds internally, so a roster change perturbs the draw sequence and only part of the noise cancels. It is enough: a 10-point season swap reads at 3.7x its own noise. **Roughly 140 season points to a win**, which puts a five-point one-week stream at ~0.04 wins — real, near the edge of resolution, and why lineup gain stays on screen beside it.

**Three bugs the tests caught, all of the plausible-wrong-answer kind:**

- **IR slots counted as open roster spots.** A full 12-man roster looked like it had two spaces, so every recommendation came back "free" and no drop was ever named — the worst direction, since a free add needs no justification and a costly one does.
- **Lineup gain cascades.** A receiver beating WR2 by 0.2 pushes WR2 into the flex, which pushes the flex RB out: the lineup gains 1.2. Comparing an add against the man he directly replaces understates every recommendation.
- **Roster order reaches the simulator.** Appending the added player instead of putting him in the dropped player's slot shifted every later player onto a different draw, and the paired comparison silently became unpaired. **A no-op swap read 0.05 wins** until this was fixed — and only an exact-zero assertion catches that.

**`choose_starters` extracted rather than copied.** Three implementations of the lineup greedy already existed and each returned only a total; this needed to know *which* players started, since the drop is by definition someone who did not. It now lives in `roster_eligibility` beside the slot taxonomy it walks. `roster_score`'s copy stays — it is in the auction bidding inner loop — and the docstring says so rather than leaving three copies unremarked.

**Verified as far as it can be pre-draft.** The tool reports "No rosters yet — the draft has not happened" and exits cleanly, which is correct on 2026-08-26. The ingest half was checked live: 60 free agents, all with week-1 projections at 0.5/reception, Nacua and Chase and McCaffrey arriving as QUESTIONABLE. Two end-to-end tests drive the recommender from ESPN-shaped payloads through the real parsers, because the seam between ingest and logic is where the web UI's defects lived.

**Reviewed to green (`/loop-review`, PR #158, 2026-08-27).** 40 findings, two critical, and both criticals were the same shape: a thing the SPEC asserts that the CODE did not do. Expected wins was written down as the objective and implemented as a displayed column — the ranking, the shortlist and the default run were all lineup gain, and `helps` was defined and never read. And stage 2 received the raw pool, so `project_league_standings` — which knows nothing about `injury_status` — simulated a player on IR at full strength, making the headline number health-blind on the branch's motivating case. **The lesson to carry: a spec sentence is not a test.** Both were stated in §5 and §6 and neither was pinned by anything.

Three more the spec had not anticipated, all plausible-wrong-answer rather than crashing: a free add was never paired (it grows the roster, and the comparison ran against the unpadded baseline, so exactly the no-drop recommendations carried double the noise while being printed against the paired threshold); IR players were counted as occupying active roster spots AND as holding starting slots, so the morning after an injury the tool said "nothing on the wire would change your lineup"; and a player the pool could not price was priced at 0.0, making kickers and just-signed backs the cheapest leftover by construction. Stage 2 also split into `midseason/swap_impact.py`, and `weekly_projections_by_espn_id` / `remaining_points_by_espn_id` moved out of the script into `midseason/waivers.py` where tests can reach them.

**Next:** after the draft, run it for real — that is when the output gets read and the columns get judged. Then the dashboard page (step 10, deferred: it is a thin presentation of the same objects over the existing column registry, and cannot be looked at until there is a roster). Then #154 (trade analyzer).
...
**The one real bug the pages could have shipped with: ROS was not ROS.** The pool's `season_mean_fpts` is a FULL-SEASON figure, and it was going straight onto the page under "projected points for the rest of the season" — roughly double the truth at week 10, with `ros_rank` inheriting it and the two pages disagreeing about the same player. `rest_of_season.remaining_totals` subtracts what he has already scored. Note it is deliberately **not** its sibling `rest_of_season_pool`: that converts to a full-season-equivalent PACE because the variance model divides by a fixed `SEASON_GAMES`. A pace is right for the simulator and wrong to print. Same input, two callers, two different correct answers.
...
**The lesson worth keeping: the only code on the branch without a view-model test held eight of the review's findings.** The My Team assembly lived inline in `routes/team.py`, reachable only through an HTTP request that first made a live ESPN call — so nothing tested it, and every defect in it was invisible. Lifting it out was the first fix and the one the rest depended on. This is the second time on this project that "untestable" and "wrong" turned out to be the same set of lines.

**It then had to move a second time, which is its own lesson.** The first lift put it in the view model, where it could at least be tested — but a module that both parses ESPN payloads and formats table cells has two halves, and the *seam* is where the next round of defects sat: a frame the presenter cleaned was read by the pipeline one step earlier (a 500 on a player reclassified mid-season), and a docstring in the presenter described the pipeline the *other* page uses. It lives in **`src/projections/midseason/my_team.py`** now, beside `project_league_standings`, so both pages are one shape: a domain function returning a `*Run`, and a view model that renders it. "Testable" was necessary and not sufficient; the layer had to be right too.

**Two other new modules worth knowing about.** `src/projections/web/views/columns.py` is the column registry (§5 of the spec) — key, label, tooltip, precision, direction, plus `Cell` and the colour scale both pages share. `src/projections/web/inputs.py` is the pre-flight "what does this page need on disk" check, shared because the two routes had a copy each and they had already diverged: one checked the ESPN credentials and the other did not.
...
**Cut from the spec, recorded there (§10):** the Trend section (needs two accumulated weekly snapshots; there are zero), the standings header's stale-data note (the page pulls live, so there is no snapshot to be stale), the rank badge (cannot be verified without a browser, and until Week 1 the whole page is verified by tests), the YTD/ROS/Total basis toggle (spec already called it an enhancement), and an undefined `Δ` column (rank-change against what? needs a stored history the repo does not keep). The first two come back together once the season starts writing snapshots.
...
**One fixture change worth flagging, because three suites share it.** `tests/test_midseason/conftest.py` declared a 6-starter lineup (`lineupSlotCounts`) while building rosters that started 4 and benched 2 — the declared lineup could not be filled by the league's own players, so any test asserting on slot coverage would have passed or failed for the wrong reason. The counts are now DERIVED from the roster the fixture builds, which changes the shared synthetic league from 6 starters to 4. Every `test_midseason` and `test_web` test now exercises that league.
