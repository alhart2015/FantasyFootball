...
**Reuse, don't duplicate (senior-dev override).** Extract the **surplus-split → round → drift-correct → floor-protect core** out of `generate_auction_values` into a shared private helper:

```
_allocate_surplus(value_signal: pd.Series, config: LeagueConfig) -> pd.Series  # Int64 dollars
```

**Exact extraction boundary:** the helper body is `auction.py:61–90` — the `if positive_vorp_sum > 0 … else (uniform)` branch (61–66), `min_bid + extra` and rounding (68–69), the full drift-correction loop with floor protection (71–89), and the final `.astype(pd.Int64Dtype())` (90). Lines **57–59 stay in the caller** — `pool_df = vorp_table.loc[pool_mask].copy()`, `positive_vorp = pool_df["vorp"].clip(lower=0.0)`, and the `positive_vorp_sum` it implies are VORP-specific selection, *not* the reusable core. The helper takes an already-computed non-negative `value_signal` (the caller passes whatever weight it wants) plus config, and returns an `Int64` dollar Series ≥ `min_bid` summing to `total_budget`.

**Index/op-order contract (load-bearing for byte-identity):** the helper **preserves `value_signal`'s index** (it reindexes nothing, resets nothing). The drift loop uses `rounded.loc[idx]` and `fractional.sort_values(...).index`, so the result's row order and *which* rows absorb drift depend on the input index — the caller must pass a Series indexed exactly as the original `pool_df.index`. The float computations (`value_signal / value_signal.sum() * surplus`, the `.round()`) stay bit-for-bit identical (same expression, same order). The helper returns `Int64` on **every** branch, including the all-zero uniform fallback (which feeds the same 68–90 path).

Two callers:
- `generate_auction_values` (hero side, unchanged behavior): builds `value_signal = pool_df["vorp"].clip(lower=0.0)` as today, then calls the helper. Output must be **byte-identical** to before (an equivalence test pins this — see Testing).
- `espn_anchored_bot_prices` (new, bot side): builds `value_signal = pool_df["espn_auction_dollars"].astype("Float64").fillna(0).clip(lower=0.0)` (NA → 0 weight → parks at `min_bid`), then calls the helper.
...
The ESPN-vs-ours readout is computed **in `tournament_cli.py`**, not inside `run_auction_tournament` — which returns only `AuctionTournamentResult` (summaries/paired-diffs/scalars) and would discard any baseline frame. The CLI already holds `pool` + `config`, so in `espn` mode it computes:
...
R1. `_allocate_surplus(value_signal, config)` is extracted from `generate_auction_values` (**body = lines 61–90**: surplus split, rounding, drift correction, floor protection, all-zero uniform fallback). It **preserves `value_signal`'s index**, returns `Int64` on every branch, and the caller retains the value-specific selection (lines 57–59). `generate_auction_values` builds `value_signal = pool_df["vorp"].clip(lower=0.0)` and produces **byte-identical** output to before (pinned by an equivalence test).
