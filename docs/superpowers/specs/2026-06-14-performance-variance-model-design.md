...
- **Realistic lineup-set-by-projection** in Consumer A: v1 keeps the existing optimal-lineup fill (now by sampled points = perfect weekly start/sit, optimistic). Setting lineups by projection and scoring by sampled (no hindsight) is a deferred refinement.
...
- **Consumer A** — `season_value.py`: replace the deterministic `per_game` fill in `_vectorized_lineup_points` / `expected_season_points*` / `marginal_season_values` with model-sampled weekly points. The weekly fill **remains the optimal legal lineup, now ranked by the sampled weekly points** (perfect weekly start/sit) — the natural extension of the existing optimal-lineup fill, and the source of the "upside option value" (a surprise bench boom can be started, a dud benched). This is a known **optimistic** assumption; realistic lineup-set-by-projection is a deferred refinement (see non-goals). The current fast-path factorizes weeks because `per_game` is uniform; per-week sampled points break that factorization, so the fill runs per `(sim, week)` over the `(n_sims, weeks, players)` sampled-points array. CRN/antithetic structure (shared draws across base and candidate rosters in `marginal_season_values`) is preserved by sharing the rng draws.
