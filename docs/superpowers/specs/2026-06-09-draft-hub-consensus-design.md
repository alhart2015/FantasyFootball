...
Input: a validated `ConsensusProjectionSchema` frame (one snapshot — single `season`, single
`asof`). Output: a `ProjectionSeasonSchema`-validated frame (the contract `generate_vorp_table`
accepts).
...
- **Filter to `has_points == True`.** VORP needs a non-null season-points total; `projected_points_ppr`
  ...
  far larger than any single-league draftable universe.
...
- `n_weeks = 1` (placeholder; the season total is not a per-week aggregate). Schema requires `1..22`.
...
Consensus-mode flow:
1. Read the consensus partition (`read_partition(..., asof=...)` or `read_latest_partition`),
   `ConsensusProjectionSchema.validate(...)`.
2. `season_proj = consensus_to_season_projections(consensus)` (§3.1).
3. `vorp = generate_vorp_table(season_proj, league_config)` (unchanged function).
4. **Left-join `consensus_adp`** from the consensus snapshot onto `vorp` by `gsis_id`, then
   `VorpTableSchema.validate(...)` (now includes the optional column).
5. Write CSV/parquet (existing `_write_output` logic) + the existing per-position stdout summary.
...
- Else, **within each position**, over the subset of rows with non-null `consensus_adp`:
  - `adp_rank` = rank of `consensus_adp` ascending (lower ADP = earlier pick = rank 1),
  - `vorp_rank` = rank of `vorp` descending (computed over the same non-null-ADP subset so the two
    ranks share one population),
  - `adp_delta = adp_rank − vorp_rank`.
