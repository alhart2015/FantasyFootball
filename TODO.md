...
### 49. Auction Strategy Tournament (Auction Slice 1) — spec written, plan + build pending

**Spec:** `docs/superpowers/specs/2026-06-17-auction-strategy-tournament-design.md` (branch `claude/pr-76-rebase-merge-606mfs`). Opens the auction branch of the Draft Hub via the snake harness's deferred simulate→score→compare seam. **Offline bake-off of three bid models as contestants:** v1 `StaticDollarBid` (static SOS `auction_dollars`), v2 `InflationBid` (live surplus inflation), v3 `MarginalValueBid` (marginal optimal-lineup lift ÷ market points-per-dollar). Shared "nominate highest remaining dollar" nominator + noisy-WTP bot market + `feasible_max = budget − min_bid·(open_slots−1)` reserve clamp isolate the bid variable; second-price+min_bid English clearing; one seeded RNG → paired "same room, different me" counterfactual; reuses `StartersValuer`/`SeasonValuer` + a promoted shared `assistant/_compare.py` bootstrap; no new schema. **Next:** plan → plan-review → execute (to `main` via PR). **Key scope cuts to honor in the plan:** v3 ships a cheap starters-marginal surrogate in the tournament (full availability-aware `SeasonValuer` marginal reserved for the future live assistant — per-candidate MC infeasible in the seeds×nominations×sims loop, per the depth-aware-strategy ≈9 hrs/slot lesson); strategic nomination deferred to Slice 2; bots are value-rational noisy-WTP (a better human-opponent model is the shared TODO #46 lever).
