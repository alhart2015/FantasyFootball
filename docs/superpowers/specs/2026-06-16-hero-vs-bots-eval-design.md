...
**Common random numbers (CRN):** the league seed passed to `simulate_league` for a cell is **`base_seed + seed_index`** — a function of the seed index **only**, independent of both `strategy` and `seat`. So for a fixed `(seat, seed)`, every strategy runs with the identical schedule and identical bot ADP-noise draws; the hero's different picks ripple into bot availability (that *is* the real snake-draft dynamic, not a CRN violation), but the opponent decision process and schedule are held fixed, so cross-strategy differences at a given `(seat, seed)` are **paired**. Because the seed is also seat-independent, comparisons *across seats* within a seed index are likewise paired (same schedule/bot draws, hero just sits elsewhere). This seat-and-strategy-independent seed derivation is the load-bearing invariant that makes "strategy X vs strategy Y, each vs the same bots" a low-variance comparison — a planner must not fold `strategy` or `seat` into the seed.

**What each cell records.** `simulate_league` returns one `LeagueResult` per seat. The cell keeps **only the hero seat's** result (the single non-`bot` label), keyed by the hero strategy. The bot seats are **not persisted** — they're redundant and would bloat/entangle the store (across hero-seat values for a seed the bots' rosters differ as the hero's picks shift, so they're not a stable field). They're also unnecessary: H2H win% is ~zero-sum (each game one W/L) and there's exactly one champion per league, so the bot field's average rates are the **mechanical complement** of the hero's — `bot_avg_win% = (50%·n − hero_win%)/(n−1) ≈ 50%`, `bot_avg_champ% = (1 − hero_is_champ)/(n−1)`. So the **bot/average-team baseline is computed from the hero results at report time** (≈50% win), not stored. Decision signal: **each strategy's absolute rate, compared *across strategies*** (paired via CRN) — not a "beats the bot" claim (the bot is mechanically ~50%).
...
**Feasibility (resumability is necessary, not sufficient).** The sweep is `n_teams × N_seeds × 6` cells. The 3 analytic strategies (`raw_vorp`, `now_or_never`, `now_or_never_floored`) are cheap; the 3 MC strategies (`season_value`, `season_value_var`, `season_value_timing`) carry per-pick season-value Monte-Carlo and **dominate cost**. To keep the run tractable, the sweep uses a **reduced `strategy_n_sims` (~50–100)** — the MC here only needs to *rank* strategies, not produce final-grade CIs (the fast-path `_vectorized_lineup_points` keeps each MC draft to ~seconds). The runner accepts `strategy_n_sims` and (optionally) a per-strategy or analytic-vs-MC seed split so the cheap strategies can run at higher `N` than the expensive three if desired.
...
- **Aggregation/report** (`hero_aggregate` + CLI subcommand) — read the per-cell (hero-only) results → validated `HeroResultSchema` frame → **headline:** per-strategy seat-averaged table (ACTUAL + PROJECTED, bootstrap CIs) plus a **computed bot/average-team baseline row** (the mechanical complement, ~50% win — derived from the hero rates, not stored); **plus** a per-seat breakdown (group by `(strategy, seat)`); **plus** paired-diff CIs of each strategy vs a `--reference` strategy (**default `now_or_never`**), paired on the shared `(seat, seed)` grid. Prints the headline table and writes the consolidated results parquet.
- **`HeroResultSchema`** — long-format results contract (`season:int, strategy:str, seat:int, seed:int, scoring:str∈{actual,projected}, wins:int, losses:int, made_playoffs:bool, is_champion:bool, points_for:float`), defined in `schemas.py` per the canonical-schema convention, validated at the consolidation boundary.
- **Results persistence (parquet I/O).** Per-cell results are resumable **JSON checkpoints** (reusing `checkpoint.py`'s `dump_results`/`load_results`, the established backtest pattern — outside the canonical store, like the H2H checkpoints). The **consolidated** `HeroResultSchema` parquet is a **derived report artifact, written directly** (not via `store.write_partition`) under a results dir (e.g. `data/backtest/hero_eval/`) — an explicit, documented exception to the store-only-I/O rule, on the same footing as the existing checkpoint JSON (the store layer is for ingested canonical data, not backtest outputs). It is never read back by ingest/feature code.
...
- **Phase 4 (data run)** — run the eval for all 6 strategies on 16-team half-PPR, 2024 + 2025, **headline `N_seeds ≈ 40–60`** (× 16 seats → 640–960 hero samples/strategy), **sweep `strategy_n_sims ≈ 50–100`** for the MC three, `now_or_never_floored` at its **shipped default F=40/λ=1**. Write a results section in `reports/draft_strategy_tests.md` (seat-averaged headline + notable per-seat findings, incl. whether `season_value_var ≈ season_value`); update PM/TODO. No cross-strategy adopt/reject verdict (decide-at-end rule). Data is present in the main checkout (`weekly_stats`/`schedules`/`external_projections` 2021–2025 verified). Run via the resumable runner in PowerShell with `KMP_DUPLICATE_LIB_OK=TRUE` + single-thread BLAS.
