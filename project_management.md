...
- **The one robust finding: `now_or_never_floored` is the deployment winner** — pooled paired **+2.35 win% vs nn [CI +1.72, +2.98]**, ahead in **4/5** seasons, season-best in 3. The scarcity floor (TODO #42) works as a real draft strategy, not just in the mixed field.
...
**Gates:** `pytest -k "draft or ingest or store or schemas"` 581 passed; `mypy src tests` clean (312); ruff clean. Each task TDD. **No cross-strategy adopt/reject verdict** (decide-at-end), but the robust takeaway is **draft with the floor**. **Deferred (top lever):** a better human-opponent model than noisy-ADP — the whole eval rests on the bots (TODO #46); also chunked/parallel sweep; per-slot recommendations; extend N beyond 25.
