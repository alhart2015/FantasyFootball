"""Cross-position smoke for LightGBMTunedModel — Plan 5b.

Single fit + predict for each of the 4 positions on synthetic fixtures, driven
by `POSITION_DISPATCH[position].factories["lightgbm-tuned"]`. Catches regressions
...
def test_lightgbm_fit_predict_smoke(position: Position) -> None:
    ...
    factory = POSITION_DISPATCH[position].factories["lightgbm-tuned"]
    model = factory()
